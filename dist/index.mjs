...
var _ScatterChart = class {
  ...
  setOptions(options) {
    this.options = {
      axis: merge2(defaultAxisOption, options.axis),
      data: [...defaultDataOption, ...options.data],
      legend: merge2(defaultLegendOption, options.legend)
    };
  ...
  setAxisOption(axisOption) {
    this.setOptions(merge2(this.options, { axis: axisOption }));
    this.setPadding();
    this.setRatio();
    this.guide.setOptions({
      padding: this.padding,
  ...
  async toBase64Image() {
    ...
    const containerCanvas = await html2canvas(document.querySelector(`.${_ScatterChart.SCATTER_CHART_CONTAINER_CLASS}`)).then((canvas) => canvas);
